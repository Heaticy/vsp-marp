...
import { buildContentReviewPrompt } from './lib/content-review'
import { screenshotHtml } from './lib/screenshot'
import { buildVisualReviewPrompt } from './lib/visual-review'
...
async function main() {
  ...
  const summary = {
    ...
    instructions: [
      '将 visual-review-prompt.md 交给当前 Agent 的多模态能力或独立审阅 Agent',
      '将 content-review-prompt.md 交给当前 Agent 或独立审阅 Agent',
      '审阅器需要直接读取提示中列出的截图文件',
