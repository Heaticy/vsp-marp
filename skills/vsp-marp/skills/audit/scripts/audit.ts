...
function parseArgs(argv: string[]) {
  ...
  return { inputFile, htmlFile, practiceRef, outputPath, severity }
}
...
Options:
  --html <file>          Rendered HTML used for page-count and layout checks
...
const htmlPath = args.htmlFile ? resolve(args.htmlFile) : ''
...
const layoutResult = auditLayoutCompliance(inputPath, htmlPath)
const contentResult = auditContentCompleteness(inputPath, htmlPath)
...
const visualScore = 70 // provisional baseline until multimodal review is merged
...
for (const d of layoutResult.deductions) {
  if (d.points <= -15)
    majorIssues.push(`${d.detail} (-${Math.abs(d.points)})`)
