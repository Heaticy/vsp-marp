...
export function compareWithReference(
  ...
  for (const ref of refFeatures) {
    ...
    // Structural features
    if (targetFeatures.hasCover === ref.hasCover) {
      sim += 15
      totalChecks += 15
    }
    if (targetFeatures.hasLastpage === ref.hasLastpage) {
      sim += 15
      totalChecks += 15
    }
    if (targetFeatures.hasToc === ref.hasToc) {
      sim += 10
      totalChecks += 10
    }
    if (targetFeatures.hasTrans === ref.hasTrans) {
      sim += 10
      totalChecks += 10
    }
