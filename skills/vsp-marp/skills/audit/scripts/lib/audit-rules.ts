...
export function auditLayoutCompliance(markdownPath: string, htmlPath: string): HardRuleResult {
  ...
  let mdContent = ''
  try {
    mdContent = readFileSync(markdownPath, 'utf-8')
  }
  catch {
    // Missing markdown is reported by downstream deductions.
  }
  try {
    readFileSync(htmlPath, 'utf-8')
  }
  catch {
    // HTML is optional for lightweight markdown checks.
  }
  ...
  if (hasLastpage) {
    ...
    const hasIcons = mdContent.includes('class="icons"')
    if (!hasThankYou) {
...
export function auditContentCompleteness(markdownPath: string, htmlPath: string): HardRuleResult {
  ...
  if (hasCover) {
    ...
    result.deductions.push({ rule: 'missing-cover', points: -25, detail: '缺少封面' })
    result.score -= 25
  }
  ...
  if (hasToc) {
    ...
    result.deductions.push({ rule: 'missing-toc', points: -10, detail: '缺少目录页' })
    result.score -= 10
  }
  ...
  if (hasLastpage) {
    ...
    result.deductions.push({ rule: 'missing-lastpage', points: -25, detail: '缺少尾页' })
    result.score -= 25
  }
  ...
  if (sectionCount >= 3) {
    ...
    result.deductions.push({ rule: 'few-sections', points: -10, detail: `章节过少(${sectionCount})` })
    result.score -= 10
  }
  ...
  if (slideCount >= 5) {
    ...
    result.deductions.push({ rule: 'few-slides', points: -10, detail: `页数过少(${slideCount})` })
    result.score -= 10
  }
